...
TODO: what does -1 duration mean? please check in unipark documentation!
      --> -1 does mean that the participant interrupted the survey and later completed it. For these participant the value of the dispcode value is "Beendet nach Unterbrechung (32)". 
...
dat$positiveFeeling <- as.numeric(as.character(dat$positiveFeeling))
dat$initiative <- as.numeric(as.character(dat$initiative))
dat$future <- as.numeric(as.character(dat$future))
dat$feelingDown <- as.numeric(as.character(dat$feelingDown))
dat$enthusiasm <- as.numeric(as.character(dat$enthusiasm))
dat$selfworth <- as.numeric(as.character(dat$selfworth))
dat$meaning <- as.numeric(as.character(dat$meaning))

# Create the new variable 'depression' by summing up the specified columns and multiplying by 2
dat$depression <- 2 * (dat$positiveFeeling + dat$initiative + dat$future + 
                            dat$feelingDown + dat$enthusiasm + dat$selfworth + 
                            dat$meaning)
summary(dat$depression)

###missings depression --> calculation only for 22 people possible, 1%???
...
dat$jobSatisfaction <- as.numeric(as.character(dat$jobSatisfaction))
...
dat$jobSatisfaction_z <- (dat$jobSatisfaction - survey_mean_js) / survey_sd_js
...
dat$burnout <- as.numeric(as.character(dat$burnout))
...
# Convert 'depression' to numeric in case it's not already
dat$depression <- as.numeric(as.character(dat$depression))
