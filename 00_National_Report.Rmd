...
ggplot(institution_counts, aes(x = reorder(inst, -Members))) + 
  geom_bar(aes(y = Members), stat = "identity", fill = "gray", alpha = 0.6) + 
  geom_bar(aes(y = count), stat = "identity", fill = "blue", alpha = 0.6) + 
  geom_text(aes(y = Members, label = sprintf("%.1f%%", response_rate * 100)), vjust = -0.5, size = 3) +
  labs(x = "Institution", y = "Number of Members (in gray) and \n Responses (in blue)") + 
  theme_minimal() + 
