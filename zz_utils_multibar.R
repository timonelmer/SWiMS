...
swims.plot.multibar <- function(
  ...
  # Example 
  # target_institution <- levels(dat$institution)[11]
  # var <- "stress_supervisor"
  ...
  if(!is.null(institution_prov) & !is.null(divider)){ # Instituion and divider
    ...
    if(!is.null(cut.small.groups) && cut.small.groups > 0){
      ...
      obj <- lapply(1:length(obj), function(sub_plot_data){
        ...
        # obj[[sub_plot_data]]$asterix <- "NO" # zk v2
        ...
        if(any(side_obj2$count <= individual_cut)) {
            ...
            if("drop" %in% names(obj[[sub_plot_data]])){
              ...
              if(length(problem_value1) > 0){
                ...
                for(kk in 1:length(problem_value1)){
                  ...
                  # obj[[sub_plot_data]]$drop[obj[[sub_plot_data]]$value == problem_value1[kk] & obj[[sub_plot_data]]$divider == problem_divider1[kk]] <- "drop" # zoran version
                  
                  # obj[[sub_plot_data]]$drop[obj[[sub_plot_data]]$divider == problem_divider1[kk]] <- "drop" # timon version
                  
                  obj[[sub_plot_data]]$count[obj[[sub_plot_data]]$group == institution_prov & obj[[sub_plot_data]]$value == problem_value1[kk] & obj[[sub_plot_data]]$divider == problem_divider1[kk]] <- obj[[sub_plot_data]]$count[obj[[sub_plot_data]]$group == institution_prov & obj[[sub_plot_data]]$value == problem_value1[kk] & obj[[sub_plot_data]]$divider == problem_divider1[kk]] + sample(c(-1, -1, 0), 1) # zoran version 3 
                  # obj[[sub_plot_data]]$count[obj[[sub_plot_data]]$group == institution_prov & obj[[sub_plot_data]]$value == problem_value1[kk] & obj[[sub_plot_data]]$divider == problem_divider1[kk]] <- obj[[sub_plot_data]]$count[obj[[sub_plot_data]]$group == institution_prov & obj[[sub_plot_data]]$value == problem_value1[kk] & obj[[sub_plot_data]]$divider == problem_divider1[kk]] + sample(c(-1, 0, 1), 1) # zoran version 2 
                  # obj[[sub_plot_data]]$asterix[obj[[sub_plot_data]]$group == institution_prov & obj[[sub_plot_data]]$value == problem_value1[kk] & obj[[sub_plot_data]]$divider == problem_divider1[kk]] <- "YES" # zoran version 2
                }
                
                obj[[sub_plot_data]]$drop <- ifelse(obj[[sub_plot_data]]$drop != "drop", "keep", obj[[sub_plot_data]]$drop) # keep previous drops
              }
              ...
              if(length(problem_value1) > 0){
                ...
                # obj[[sub_plot_data]]$drop <- ifelse(obj[[sub_plot_data]]$value %in% problem_value1 & obj[[sub_plot_data]]$divider %in% problem_divider1, "drop", "keep") # Zoran version
                
                # obj[[sub_plot_data]]$drop <- ifelse(obj[[sub_plot_data]]$divider %in% problem_divider1, "drop", "keep") # timon version
          ...
          if(all(side_obj$count > cut.small.groups) & all(side_obj2$count > individual_cut)){
            
            obj[[sub_plot_data]]$drop <- "keep"
            
          }
          
          obj[[sub_plot_data]]$proportion <- obj[[sub_plot_data]]$count/obj[[sub_plot_data]]$n_total  # zk v2
    ...
    if("n_total" %in% colnames(plot_data)){
      ...
      # # zk v2
      # plot_data$n_total <- ifelse(plot_data$asterix == "YES", paste0(plot_data$n_total, "*"), plot_data$n_total)
      # 
      # daten_mit_sternchen <- plot_data %>%
      #   
      #   # Filtern: Wende die Logik nur auf diese Gruppe an
      #   filter(group == institution_prov) %>%
      #   
      #   # Nach 'value' gruppieren, wie zuvor
      #   group_by(value) %>%
      #   
      #   # Sternchen-Logik anwenden
      #   mutate(
      #     # Prüfen, ob *irgendein* Wert in n_total in dieser Gruppe ein "*" enthält
      #     gruppe_hat_sternchen = any(str_detect(n_total, "\\*"), na.rm = TRUE),
      #     
      #     # n_total bedingt ändern:
      #     n_total = if_else(
      #       # Bedingung 1: Die Gruppe MUSS ein Sternchen enthalten
      #       gruppe_hat_sternchen &
      #         
      #         # Bedingung 2: Der aktuelle n_total-Wert darf NICHT NA sein
      #         !is.na(n_total) &
      #         
      #         # *** NEUE BEDINGUNG: Der aktuelle n_total-Wert darf NOCH KEIN Sternchen haben ***
      #         !str_detect(n_total, "\\*"),
      #       
      #       # Wenn ALLE Bedingungen erfüllt: Sternchen anhängen (paste0)
      #       paste0(n_total, "*"),
      #       
      #       # Wenn Bedingungen NICHT erfüllt: n_total beibehalten
      #       n_total
      #     )
      #   ) %>%
      #   
      ...
      #   select(-gruppe_hat_sternchen)
      # 
      # # 2. Die ursprünglichen Daten ohne die betroffene Gruppe auswählen
      # andere_daten <- plot_data %>%
      #   filter(group != "invest_prov")
      # 
      # # 3. Die beiden Datensätze wieder zusammenfügen (bind_rows)
      # plot_data <- bind_rows(daten_mit_sternchen, andere_daten)
      # 
      ...
      if(space4comp){
        ...
        g <- ggplot(plot_data, aes(x = x_pos, y = proportion, fill = value_fill, alpha = group)) + 
          ...
          theme(
            ...
            panel.grid.major.x = element_line(color = "gray69", size = 0.5),
